...
{
	// rights for editing current element will be in element get filterSECTION_LIST
...
if ($bAllowAccess)
{
	...
	while ($arSection = $rsIBlockSectionList->GetNext())
	{
		...
		if ($arSection["DEPTH_LEVEL"] == "1") {
			$arResult["SECTION_PARENT_LIST"][$arSection["ID"]] = array(
				"VALUE" => $arSection["NAME"],
				"DEPTH_LEVEL" => $arSection["DEPTH_LEVEL"]
			);
		}
	...
	$arResult["PROPERTY_LIST_FULL"] = array(
		...
		"IBLOCK_PARENT_SECTION" => array(
			"PROPERTY_TYPE" => "L",
			"ROW_COUNT" => "12",
			"MULTIPLE" => $arParams["MAX_LEVELS"] == 1 ? "N" : "Y",
			"ENUM" => $arResult["SECTION_PARENT_LIST"],
		),
		...
		"IBLOCK_SECTION" => array(
			...
			"ROW_COUNT" => "12",
			"MULTIPLE" => $arParams["MAX_LEVELS"] == 1 ? "N" : "Y",
			"ENUM" => empty($arParams['ID']) ? [] : $arResult["SECTION_LIST"],
		),
		...
		/*
		"UF_PERFORMANCE" => array(
			"PROPERTY_TYPE" => "S",
			"MULTIPLE" => "N",
			"COL_COUNT" => $COL_COUNT,
		),*/
		"UF_ENERGY_EFFICIENCY" => array(
			"PROPERTY_TYPE" => "S",
			"MULTIPLE" => "N",
			"COL_COUNT" => $COL_COUNT,
		)
	...
	if (!empty($arParams['ID'])) {
		$catalogProduct = CCatalogProduct::GetByID(intval($arParams['ID']));
		$arResult['CATALOG']['AVAILABLE_QUENTITY'] = $catalogProduct['QUANTITY'];
		$arPrices = [];
		...
		while ($price = $price_result->Fetch())
		{
			$arPrices[]= $price;
		}
		$arResult['CATALOG']['EXT_PRICES'] = $arPrices;
	}
...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		// custom ads properties
		$arOwnProperties = ['AVAILABLE_QUENTITY', 'EXT_PRICE'];
		foreach ($arOwnProperties as $i => $propertyID) {
			$arPropertyValue = $arProperties[$propertyID];
			if ($propertyID == 'AVAILABLE_QUENTITY') {
				$arUpdateValues[$propertyID] = $arPropertyValue;
			}
			if ($propertyID == 'EXT_PRICE'){
				$arUpdateValues[$propertyID] = $arPropertyValue;
			}
		}
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			if ($arParams["ID"] > 0)
			{
				...
				// обновить элемент
				$res = $oElement->Update($arParams["ID"], $arUpdateValues, $bWorkflowIncluded, true, $arParams["RESIZE_IMAGES"]);
				if (!$res)
				{
					$arResult["ERRORS"][] = $oElement->LAST_ERROR;
				}
				else {
					// обновить доступное кол-во в торговом каталоге
					if (!empty($arUpdateValues['AVAILABLE_QUENTITY'])) {
						$arFields = array(
							"ID" => $arParams['ID'], 
							"QUANTITY" => intval($arUpdateValues['AVAILABLE_QUENTITY'])
							);
			
						$resUpd =\Bitrix\Catalog\Model\Product::update(intval($arFields['ID']),$arFields);
						if (!$resUpd) $arResult["ERRORS"][] = 'Ошибка при сохранении общего количества';
					}
					// обновить ценовое предложение 
						//$PRICE_TYPE_ID = 1;
						/*$arFields = Array(
							"PRODUCT_ID" => $arParams["ID"],
						...
						$hasLimit = false;
						$obPrice = new CPrice();
						foreach ($arUpdateValues['EXT_PRICE'] as $index => $extPrice) {
							if(empty($extPrice['VALUE']) && empty($extPrice['QUANTITY_FROM'])) continue;
								$arFields['PRICE'] = $extPrice['VALUE'];
								$arFields['QUANTITY_FROM'] = $extPrice['QUANTITY_FROM'];
								$arFields['QUANTITY_TO'] = $extPrice['QUANTITY_TO'];
								
								if(!$result = \Bitrix\Catalog\Model\Price::update($arParams["ID"], $arFields)){
									$arResult["ERRORS"][] = GetMessage("IBLOCK_ADD_PRICE_$index") . ' ' . $result->getErrorMessages();
								} else {
									$extLimit = $arFields;
									$hasLimit = true;
								}
						}
						if ($hasLimit && !empty($extLimit['QUANTITY_TO']) && !empty($extLimit['PRICE'])) {
							$extLimit['QUANTITY_FROM'] = intval($extLimit['QUANTITY_TO']) + 1;
							$extLimit['QUANTITY_TO'] = '';
							$obPrice->Add($extLimit,true);
						}
				}
			}
			// add new element
			else
			{
				...
				// если товар создался
					...
					// добавляем параметры торгового каталога
					if (!empty($arUpdateValues['AVAILABLE_QUENTITY'])) {
						$arFields = array(
							"ID" => $arParams['ID'], 
						...
						$resAdd = \Bitrix\Catalog\Model\Product::add($arFields);
						if (!$resAdd) $arResult["ERRORS"][] = 'Ошибка при сохранении общего количества';
					}
					// ценовое предложение 
						$PRICE_TYPE_ID = 1;
						$arFields = Array(
						...
						foreach ($arUpdateValues['EXT_PRICE'] as $index => $extPrice) {
							if(empty($extPrice['VALUE']) && empty($extPrice['QUANTITY_FROM'])) continue;
								...
								if(!$res = $obPrice->Add($arFields,true)){
									$arResult["ERRORS"][] = GetMessage("IBLOCK_ADD_PRICE_$index") . ' ' . $APPLICATION->GetException();
								} else {
									$extLimit = $arFields;
									$hasLimit = true;
								}
