...
class CBitrixPersonalOrderDetailComponent extends CBitrixComponent
{
	...
	// Получить позиции материалов проверки
	protected function getJSON(){
		$orderProps = $this->arResult['ORDER_PROPS'];
		$json = array_filter($orderProps, function($prop) {
			return $prop['CODE'] == 'JSON';
		});
		$values = current($json)['VALUE'];
		$arr = json_decode($values);
		$this->arResult['FILES_CHECK'] = $arr;
	}
	/**
