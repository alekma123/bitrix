...
(function() {
	BX.Sale.PersonalOrderComponent.PersonalOrderDetail = {
		init : function(params)
		{
			...
			// Кнопка оплату проивел
			var payed = document.querySelector('#deal_card #payed');

			// отправка материалов проверки
			var totalFormsUpload = document.querySelectorAll('.form-upload').length;

			// форма загрузки материалов проверки
			for (let index = 0; index < totalFormsUpload; index++) {
				// форма
				let form = document.forms[`upload-files-${index}`];
				// загрузка файлов
				let btn = form.querySelector(`#btn-load-files-${index}`);
				btn.addEventListener('click', function(event){
					form[`upload-files_${index}[]`].click();
				}); 
				
				// отправка запроса
				$(`#upload-files-${index}`).on('submit', function(event) {
					event.preventDefault();
					var formData = new FormData(form);
					formData.append('action', 'uploadFiles');
					formData.append('totalPosition', totalFormsUpload)
					this.sendRequest(formData);
				}.bind(this));

				// чекбокс все материалы загружены
				let submitCheckbox = form.querySelector(`#loaded_${index}`);
				submitCheckbox.addEventListener('click', function(event){
					$(`#upload-files-${index}`).submit();
				});

			}

			// Изменить статус на оплата проведена
			BX.bind(payed, 'click', function(){
				BX.ajax(
					{
						method: 'POST',
						dataType: 'html',
		...
		sendRequest_off: function(action, formData, params){
			...
			const URL = '/local/components/cwr/sale.personal.order.detail/uploadFiles.php';
			$.ajax({
                url: URL,
                type: 'POST',
                cache: false,
