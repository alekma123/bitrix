...
<div class="page">
    <div class="page-deal-card" id="deal_card">
        <div class="bg-overlay"></div>

        <div class="deal-card deal-card-seller">
            <div class="container">
                <a class="deal-adv-icon deal-adv-timer timer" id="timer_1" data-seconds="840">14:02</a>
                <a class="deal-adv-icon deal-adv-chat"><span>1</span></a>

                <a class="link-back" href="/personal/seller_lk.php">Назад</a>

                <div class="sub-title-big gold">Сделка №<?=$arResult["ACCOUNT_NUMBER"]?></div>

                <?foreach ($arResult['BASKET'] as $basketItem) :?>
					<h2>
						<?=$basketItem['NAME']?>
						<!-- <span class="gold-name">70 TH/s</span> -->
						<span class="gold-name"><?= current($arResult['BASKET'])['PROPS'][0]['VALUE'] ?></span>
					</h2>

					<div class="check-timer flex-box align-center space-between">
						<div class="check-timer-info">
							<h3>Таймер проверки</h3>
							<p>по истечении времени сделка будет автоматически отменена</p>
						</div>
                ...
                <div class="deal-info flex-box space-between">
                    <div class="deal-info-block">
                        ...
                        <div class="gold"><?=$arResult['DATE_INSERT_FORMATED']?></div>
                    </div>
                    <div class="deal-info-block">
                        <p>Номер заказа</p>
                        <div class="gold"><?=$arResult["ACCOUNT_NUMBER"]?></div>
                    </div>
                </div>

                <div class="seller-chat flex-box space-between">
                    <a class="btn_chat">Чат с покупателем</a>
                    <div class="chat-info"><span>1</span>сообщение требуют ответа</div>
                </div>

                <div class="deal-card-check-files">
                    <h3>Загрузка материалов на проверку</h3>
                    <p>Номер сделки может быть внесен в кадр (разборчиво) на смартфоне, напечатан или написан на
                        бумаге.</p>
                    <p>Запись номера осуществляется в начале файла в одном кадре с оборудованием, сохраняется одним
                        видеофайлом без редактирования, допускается запись номера в начале одного файла для нескольких
                        проверяемых устройств.</p>
                    <p>Если видео файлов несколько, для каждого записывается номер данной сделки в начале. Данное
                        действие
                        подтверждает, что оборудование реально существует, соответствует заявленным параметрам, в
                        надлежащем
                        качестве и комплектации, данная видеопроверка уникальна и выполнена в данный момент для данного
                        клиента.</p>
                    <p>Файл видеопроверки без кода номера сделки не действителен.</p>
                </div>
				<?#Материалы проверки?>
				...
				<?for ($i=0; $i < intval($quantity); $i++):?>
					<div class="deal-adv">
						...
						<div class="deal-adv-amount"><span><?=$i+1?></span> из <?=$quantity?></div>
						<div class="caption"><?=current($arResult['BASKET'])['NAME']?>, <?=current($arResult['BASKET'])['PROPS'][0]['VALUE']?></div>
	
						<?#кнопка загрузить?>
						<form class="form-upload" id="upload-files-<?=$i?>" name="upload-files-<?=$i?>" enctype="multipart/form-data">
							<input type="hidden" name="orderID" value="<?=$arResult['ID']?>">
							<input type="hidden" name="position" value="<?=$i?>">							
							<a id="btn-load-files-<?=$i?>" class="btn btn_load btn-center">
								<input type="file" name="upload-files_<?=$i?>[]" class="hidden" multiple>
								Загрузить
							</a>
							<div class="form-row-psevdo load-confirm">
								<input id="loaded_<?=$i?>" type="checkbox" name="" <?if(!empty($arResult['FILES_CHECK'][$i]->filesIds)) echo "checked" ?> />
								<div class="psevdo-button checkbox"></div>
								<label for="loaded_<?=$i?>">Все материалы загружены</label>
							</div>
						</form>
                ...
                <div class="pay-get">
                    ...
                    <div class="form-row-psevdo">
                        <input id="payed" type="checkbox" <?if(!$arResult['STATUS_DEAL']['IS_CHECKED']) echo 'disabled'?> name="payed" <?if($arResult['STATUS_DEAL']['IS_PAID']) echo "checked"?> />
                        <div styel="top: 2px!important"  class="psevdo-button checkbox"></div>
                        <label for="payed">Оплата пришла</label>
                    </div>
                </div>

                <div class="deal-card-appeal">
                    <p>У вас есть жалоба по сделке? Отправьте аппеляцию</p>
                    <a class="btn-center btn btn_green">Подать аппеляцию</a>
                </div>
	...
	<div style='display:none' class="container-fluid sale-order-detail">
