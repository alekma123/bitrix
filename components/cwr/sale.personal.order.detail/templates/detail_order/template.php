...
if ( !empty($arResult['BASKET']) ) {
	$arResult['BASKET'] = array_values($arResult['BASKET']);
} 

print_r('<script>console.log(' . json_encode($arResult, JSON_UNESCAPED_UNICODE) . ');</script>');
...
?>
	...
	<div class="page">
		<div class="page-deal" id="deal">
			<div class="bg-overlay">
			</div>
			<div class="container">
				<div class="deal-navbar flex-box align-end">
					<div class="deal-navbar-item <?if($arResult['STATUS_DEAL']['IS_CHECKED']) echo 'active' ?>" id='step_001' data-id="step_0">
						<p>
							Проверка<br>
							пройдена
						</p>
					...
					<div class="deal-navbar-item <?if($arResult['STATUS_DEAL']['IS_PAID']) echo 'active' ?>" data-id="step_1">
						<p>
							Оплата<br>
							проведена
						</p>
					...
					<div class="deal-navbar-item <?if($arResult['STATUS_DEAL']['IS_DELIVERED']) echo 'active' ?>" data-id="step_2">
						<p>
							Доставка<br>
							заказана
						</p>
				...
				<div class="deal">
					<div class="deal-step step_0 <?if(!$arResult['STATUS_DEAL']['IS_CHECKED']) echo 'active'?>" data-id="step_0">
						<a class="deal-adv-icon deal-adv-chat">1</a> <!--                    <a class="deal-adv-icon deal-adv-clock"></a>--> <a class="deal-adv-icon deal-adv-timer timer" id="timer_1" data-seconds="840">14:02</a>
						<div class="sub-title-big">
							Сделка № <?=$arResult["ACCOUNT_NUMBER"]?>
						</div>
						
						<?
						$arProps = [];
						foreach ($arResult['BASKET'][0]['PROPS'] as $key => $prop) {
							$arProps[$prop['CODE']] = $prop;
						}?>

						<h2><?= $arResult['BASKET'][0]['NAME'] ?> <span class="gold-name"><?=$arProps['PERFORMANCE']['VALUE'];?> TH/s</span></h2>
						
						<div class="check-timer flex-box align-center space-between">
							<div class="check-timer-info">
								<h3>Таймер проверки</h3>
								<p>
									по истечении времени сделка будет автоматически отменена
								</p>
							</div>
							<?
							$quantity = $arResult['BASKET'][0]['QUANTITY'];
							$deal_timer = 900 + (($quantity - 1) * 300);
							?>
							<div class="check-timer-counter timer" data-seconds="<?= $deal_timer ?>" <?= $deal_timer ?><span class="bxhtmled-surrogate-inner"><span class="bxhtmled-right-side-item-icon"></span><span class="bxhtmled-comp-lable" unselectable="on" spellcheck="false"></span></span>14:02
							</div>
						</div>
						
						<div class="seller-chat flex-box space-between">
							<a class="btn_chat">Чат с продавцом</a>
							<div class="seller-status online">
								продавец сейчас онлайн
								<span></span>
							</div>
						</div>

						<?#Материалы проверки?>
				<?$quantity = current($arResult['BASKET'])['QUANTITY']?>
				<?for ($i=0; $i < intval($quantity); $i++):?>

					<?if(empty($arResult['FILES_CHECK'][$i]->filesIds)):?>
						<div class="deal-adv notloaded">
							<div class="deal-adv-notice">Продавец еще не загрузил материалы для проверки</div>
							<div class="deal-adv-slider" data-id="2"></div>
							<div class="caption">
								<?=$arResult['BASKET'][0]['NAME'] ?> , <?=$arProps['PERFORMANCE']['VALUE'];?>
							</div>
							<div class="deal-adv-amount">
								<?=$i+1?> из <?=$arResult['BASKET'][0]['QUANTITY']?>
							</div>
						</div>
						<?else:?>
							<div class="deal-adv <?if($arResult['FILES_CHECK'][$i]->isChecked
							) echo 'checked'; ?>">
								<a class="deal-adv-icon deal-adv-clock"></a>
			
								<div class="deal-adv-slider" data-id="<?=$i?>">
									<?foreach ($arResult['FILES_CHECK'][$i]->filesIds as $index => $fileId):?>
										<a href="<?=CFile::GetPath($fileId)?>" class="deal-adv-slider-item magnific">
											<img src="<?=CFile::GetPath($fileId)?>"/>
										</a>
									<?endforeach?>
								</div>
								<div class="deal-adv-slider-nav">
								...
								<div class="flex-box">
									<div class="deal-adv-slider-preview" data-id="<?=$i?>">
										<?foreach ($arResult['FILES_CHECK'][$i]->filesIds as $index => $fileId):?>
											<div class="deal-adv-slider-preview-item">
												<img src="<?=CFile::GetPath($fileId)?>"/>
											</div>
										<?endforeach?>
									</div>
									<div class="deal-adv-slider-preview-count">+ <span></span></div>
								</div>
								...
								<div class="deal-adv-checked <?if (!$arResult['STATUS_DEAL']['IS_CHECKED']) echo 'not-checked'?>" id="deal-adv-check">
									<span class="check-btn"></span>Проверено
								</div>
				...
				<?endfor?>
						...
						<div class="deal-steps-nav flex-box space-between">
							<a class="col-2 btn_grey btn" onclick="document.getElementById('step_001').classList.add('active')">Отменить</a>
							<div class="col-2">
								<a data-href="step_1" class="step-action btn_green btn <?if(!$arResult['STATUS_DEAL']['IS_CHECKED']) echo "enable"?>"> Далее</a>
								<p>
									Продолжая, вы соглашаетесь, что товар соответствует заявленному
								</p>
							</div>
						</div>

						<div class="deal-message deal-canceled">
							<a class="deal-message-close"></a>
							<div class="deal-message-container">
								<h3>Сделка аннулирована</h3>
								<p>
									Время оформления сделки закончилось. Выберите в каталоге нужную модель, чтобы оформить покупку
								</p>
							</div>
						</div>
					</div>
					<div class="deal-step step_1 <?if($arResult['STATUS_DEAL']['IS_CHECKED'] && !$arResult['STATUS_DEAL']['IS_PAID']) echo 'active' ?>" data-id="step_1">
						<div class="check-timer flex-box align-center space-between">
							<div class="check-timer-info">
								<h3>Таймер проверки</h3>
								<p>
							...
							<div class="check-timer-counter timer" data-seconds="840">
								14:02
							</div>
						</div>
						<div class="deal-info flex-box space-between">
							<div class="deal-info-block">
								<p>Дата и время сделки</p>
								<div class="gold">
						...
						<h2><?= $arResult['BASKET'][0]['NAME'] ?> <span class="gold-name"><?= $arResult['BASKET'][0]['PROPS'][0]['VALUE'] ?> TH/s</span></h2>
						<div class="deal-estimate flex-box">
							<div class="deal-estimate-item">
								Количество
								<p>
									<?= $arResult['BASKET'][0]['QUANTITY'] ?>
								</p>
							</div>
							<div class="deal-estimate-item">
								Цена
								<p>
									<?= $arResult['BASKET'][0]['BASE_PRICE'] ?> USDT
								</p>
							</div>
							<div class="deal-estimate-item">
								Сумма
								<p>
									<?= $arResult['PRODUCT_SUM'] ?> USDT
								</p>
							</div>
						</div>
						<div class="deal-wallet-method registration-method">
							<a data-href="wallet_0" class="registration-method-item choose-method-btn active"><span>Кошелёк</span></a> <a data-href="wallet_1" class="registration-method-item choose-method-btn"><span>Карта</span></a>
						</div>
						<div class="deal-wallet choose-method-block active" data-id="wallet_0">
							<div class="flex-box space-between align-center">
								<p >
									Кошелёк:
								</p>
								<div class="deal-wallet-code">
									1N4Qbzg6LSXUXyXu2MDuGfzxwMA7do8AyL <a class="deal-wallet-code-copy" onclick="copyText()"></a>
								</div>
							</div>
						</div>
						<div class="deal-wallet choose-method-block" data-id="wallet_1">
							<div class="flex-box space-between align-center">
								<p>
									Номер карты:
								</p>
								<div class="deal-wallet-code">
									<input id="wallet_card" type="text" name="" placeholder="0000 0000 0000 0000">
									<a class="deal-wallet-code-copy" onclick="copyText1()"></a>
								</div>
							</div>
						</div>

						<div class="deal-pay-check">
							Перейдите в своё приложение и совершите перевод на указанные реквизиты с указанной суммой <a class="deal-adv-icon deal-adv-clock"></a>
						</div>
						
						<div class="form-row-psevdo deal-pay-checkbox">
							<input id="payed" type="checkbox" name="payed">
							<div class="psevdo-button checkbox">
							</div>
							<label for="payed">Оплату произвел</label>
						</div>
						...
						<div class="deal-steps-nav">
							<a data-href="step_2" class="step-action btn_green btn <?if(!$arResult['STATUS_DEAL']['IS_PAID']) echo 'enable'; ?>">Далее</a>
							<p class="note-grey">
								Переход к оформлению доставки произойдёт после подтверждения оплаты продавцом
							</p>
						</div>
					</div>
					<div class="deal-step step_2 <?if($arResult['STATUS_DEAL']['IS_CHECKED'] && $arResult['STATUS_DEAL']['IS_PAID'] && !$arResult['STATUS_DEAL']['IS_DELIVERED']) echo 'active' ?>" data-id="step_2">
						<div class="deal-order">
							<form>
								<div class="form-row">
									<label for="fio">ФИО получателя</label> <input id="fio" type="text" name="" placeholder="ФИО">
								</div>
								<div class="form-row">
									<label for="phone">Телефон для связи</label> <input id="phone" type="text" name="user_phone" placeholder="+7 - ( ___ ) - ___ - __ - __">
								</div>
								<div class="form-row select-row">
									<label for="deliv">Выберите службу доставки</label>
									<select id="deliv">
										<option>СДЭК</option>
										<option>Почта Росии</option>
										<option>EMS</option>
									</select>
								</div>
								<div class="form-row">
									...
									<div class="flex-box">
										<div class="form-row-psevdo simple">
											<input data-id="0" id="getting_0" type="radio" name="getting" checked="">
											<div class="psevdo-button">
											</div>
											<label for="getting_0">доставка до двери</label>
										</div>
										<div class="form-row-psevdo simple">
											<input data-id="1" id="getting_1" type="radio" name="getting">
											<div class="psevdo-button">
											</div>
											<label for="getting_1">пункт выдачи</label>
										</div>
									</div>
								</div>
								<div class="form-row">
									<label>Адрес</label> <input data-id="0" class="deliv_address active" type="text" id="addressInput" name="" placeholder="введите адрес">
									<p data-id="1" class="deliv_address ">
										Выберите пункт выдачи на карте
									</p>
								</div>
								<div id="map" class="deal-order-map">
									<iframe src="https://www.google.com/maps/d/embed?mid=14XiuXsI3K06SZ455cfT0oP64d79545M&ehbc=2E312F" width="100%" height="300"></iframe>
								</div>
								<div class="deal-steps-nav">
									<!-- <button type="submit" class="btn_green btn">Оформить заказ<br>
									</button> -->

									<!-- Пробная кнопка -->
									<a href="/personal/deal-card.php?ORDERID=<?=$arResult['ID']?>&order_id=<?=$arResult['ID']?>" class="btn_green btn">Оформить заказ</a>
								</div>
							</form>
						</div>
					</div>
...
<script>
function copyText() {
  var text = document.querySelector('.deal-wallet-code').innerText.trim();
...
function copyText1() {
  var textField = document.querySelector('#wallet_card');
  textField.select();
  document.execCommand('copy');
  ...
  alert('Текст скопирован в буфер обмена.');
	...
	<div hidden class="container-fluid sale-order-detail">
								...
								<ul class="sale-order-detail-about-order-inner-container-details-list">
									...
									?>
										<li class="sale-order-detail-about-order-inner-container-list-item">
											...
											<a class="sale-order-detail-about-order-inner-container-list-item-link" href="mailto:<?= htmlspecialcharsbx($arResult["USER"]["EMAIL"]) ?>"><?= htmlspecialcharsbx($arResult["USER"]["EMAIL"]) ?></a>
										</li>
									<?php
									}
												...
												<div class="row sale-order-detail-payment-options-methods-information-block">
													...
													?>
														<div class="col-md-2 col-sm-12 col-xs-12 sale-order-detail-payment-options-methods-button-container">
															...
															?>
																<a class="btn-theme sale-order-detail-payment-options-methods-button-element-new-window" target="_blank" href="<?= htmlspecialcharsbx($payment['PAY_SYSTEM']['PSA_ACTION_FILE']) ?>">
