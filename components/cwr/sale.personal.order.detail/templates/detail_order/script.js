...
(function() {
	BX.Sale.PersonalOrderComponent.PersonalOrderDetail = {
		init : function(params)
		{
			...
			// Форма материалов проверки
			var dealAdv = document.querySelector('#deal-adv');
			// Кнопка проверено
			var dealAdvCheck = document.querySelector('#deal-adv #deal-adv-check');
			// Кнопка оплату проивел
			var payed = document.querySelector('#deal #payed');
			...
			// Изменить статуса на проверено
			BX.bind(dealAdvCheck, 'click', function(){
				BX.ajax(
					{
						method: 'POST',
						dataType: 'html',
				...
				BX.ajax(
					{
						...
						data:
						{
							...
							data: {isChecked: true, position: 1},
							orderData: params.paymentList,
							// orderData: params,
							templateName : params.templateName,
							returnUrl: params.returnUrl, 
