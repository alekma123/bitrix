...
<div class="page">
    <div class="page-deal-card" id="deal_card">
        ...
        <div class="deal-card">
            <div class="container">
                <a class="link-back" href="/personal">Назад</a>
				
                <div class="sub-title-big gold">Сделка №<?=$arResult["ACCOUNT_NUMBER"]?></div>
				<? foreach ($arResult['BASKET'] as $basketItem) :?>
                <h2><?=$basketItem['NAME']?><span class="gold-name"><?=$basketItem['PROPS'][0]['VALUE']?> TH/s</span></h2>
                ...
                <div class="deal-estimate flex-box">
                    ...
                    <div class="deal-estimate-item">
                        <span>Цена</span>
                        <p><?=$basketItem['PRICE']?> USDT</p>
                    </div>
                    <div class="deal-estimate-item">
                        <span>Сумма</span>
                        <p><?=$arResult['BASE_PRODUCT_SUM']?> USDT</p>
                    </div>
                </div>
				<? endforeach;?>
                ...
                <div class="seller-chat flex-box space-between">
                    ...
                    <div class="seller-status online">продавец сейчас онлайн <span></span></div>
                </div>
                ...
                <div class="delivery-address">
					...
					<?$address = array_filter($arResult['ORDER_PROPS'], function($prop){ 
						return $prop['CODE'] == 'ADDRESS';
					} )?>
					
                    <p><?=current($address)['VALUE']?></p>

                    <div class="form-row-psevdo">
                        <input onclick='showlike()' id="delivered" type="checkbox" name="delivered" <?if($arResult['STATUS_DEAL']['IS_DELIVERED']) echo "checked"?> />
                        <div class="psevdo-button checkbox"></div>
                        <label onclick='showlike()' for="delivered">Подтвердить доставку</label>
                    </div>
                </div>

				<div id='doulikeit' class="doulikeit" style='display: none;margin: 35px 0px;'>
					<div class="form-row-psevdo">
						<span>Вам понравилась сделка?</span>
							<div style='margin: 10px 0 0 0;display: flex;justify-content: space-evenly;' class="contain">
								<div class="like" id="like">
									<input id="like" type="radio" name="liked"/>
									<label onclick="check_like()" for="like">
										<img id="like_pic" class="check-filter-grey" style='width: 70px;' src="/local/templates/bineo/assets/img/like_n.svg"  alt="like">
									</label>
								</div>
								<div class="dislike" id="dislike">
									<input id="dislike" type="radio" name="liked"/>
									<label onclick="check_dislike()" for="dislike">
										<img id="dislike_pic" class="check-filter-grey" style='width: 70px;' src="/local/templates/bineo/assets/img/dislike_n.svg" alt="dislike">
									</label>
								</div>
							</div>	
					</div>
				...
				<script>
					function check_like(){
						...
						let like_pic = document.getElementById('like_pic');
						let dislike_pic = document.getElementById('dislike_pic');
						if (like.checked){
							like_pic.classList.toggle('check-filter-green');
							dislike_pic.classList.remove('check-filter-grey');
							dislike_pic.classList.remove('check-filter-green');
							dislike_pic.classList.add('check-filter-grey');
							
						}
					}, 10);
				}
				function check_dislike(){
						setTimeout (function() {
						let like = document.getElementById('like');
						let dislike = document.getElementById('dislike');
						let like_pic = document.getElementById('like_pic');
						let dislike_pic = document.getElementById('dislike_pic');
						if (dislike.checked) {
							dislike_pic.classList.toggle('check-filter-green');
							like_pic.classList.remove('check-filter-grey');
							like_pic.classList.remove('check-filter-green');
							like_pic.classList.add('check-filter-grey');
				 ...
				 function showlike(){
					...
					let doulikeit = document.getElementById('doulikeit');
					if (delivered.checked){
                        doulikeit.style.display = 'block';
                    }else{
                        doulikeit.style.display = 'none';
                    }
				 }
				</script>
